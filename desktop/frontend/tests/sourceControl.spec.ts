...
test('Source Control repo detail tabs including release Tags load real content', async ({ page }) => {
  const errors: string[] = []
  ...
  await expect(page.locator('#source-detail')).toContainText('v1.2.3')
  await expect(page.locator('#source-detail')).toContainText('Release 1.2.3')
